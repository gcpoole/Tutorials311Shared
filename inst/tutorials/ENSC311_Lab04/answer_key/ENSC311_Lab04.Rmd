...
penguins$plot_color <- year_colors[penguins$year]
```
...
```{r 01_load_penguins, exercise=TRUE}

```

```{r 01_load_penguins-solution}
library(palmerpenguins)
class(penguins)
penguins
```

```{r 01_load_penguins-code-check}
grade_code()
```

```{r 02_table_type_quiz}
quiz(
  question(
...
```{r 03_str_penguins, exercise=TRUE, exercise.setup = "pre_penguin"}

```

```{r 03_str_penguins-solution}
str(penguins)
```

```{r 03_str_penguins-code-check}
grade_code()
```
...
```{r 04_unique_values, exercise=TRUE, exercise.setup = "pre_penguin"}

```

```{r 04_unique_values-solution}
unique(penguins$species)
unique(penguins$island)
unique(penguins$year)
```

```{r 04_unique_values-code-check}
grade_code()
```
...
```{r 05_summary_penguins, exercise=TRUE, exercise.setup = "pre_penguin"}

```

```{r 05_summary_penguins-solution}
summary(penguins)
```

```{r 05_summary_penguins-code-check}
grade_code()
```
...
<!-- The following code will generate random combinations of `species` and `island` by determining all possible combinations and then selecting a random subset of those combinations.  Play with the code to figure out how it works.  For instance, you might want to insert a line of code to display `all_combos` and then run the code.  Then delete the line that displays `all_combos` and insert a line that displays `n_combos`.  If you look at each intermediate value in the code, you should be able to see how it works!

Once you are done playing, click "start over" and run all the code a few times. The point is see different examples of possible answers to the question "*Which islands have which species?*" so you have an idea of what an answer to that question would look like. 

Remember, each "answer" you get is a *potential* answer, drawn at random from all possible answers.  Once you understand what potential answers look like, you can answer the question by generating a similar table *from the `penguins` data set.*  We'll do that in a moment.  For now, play with this code, see how it works, and then run it a few times to generate some randomized potential answers.

```{r species_island_example, exercise = TRUE, exercise.setup = "pre_penguin"}
# get all possible combinations of islands and species
all_combos <- 
  expand.grid(
...
# generate a random number of combinations that will be selected
n_to_select <- sample(1:n_combos, 1)
# randomly select `selection_count` row numbers; sort to keep rows in order
selected_row_numbers <- 
  sort(
    sample(1:n_combos, n_to_select))
# select the corresponding records from 
all_combos[selected_row_numbers, c(2,1)]
```  
-->
## Who's where?
...
```{r 08_distinct_penguin_island, exercise=TRUE, exercise.setup = "pre_penguin"}

```

```{r 08_distinct_penguin_island-solution}
dplyr::distinct (
  penguins[ , c("island", "species")]
)
```

```{r 08_distinct_penguin_island-code-check}
grade_code()
```

What did you find?

```{r 09_distribution_quiz}
quiz(
  question(
...
```{r 10_plot_flipper_bill, exercise=TRUE, exercise.setup = "pre_penguin"}
 
```

```{r 10_plot_flipper_bill-solution}
plot(penguins$flipper_length_mm, penguins$bill_length_mm)
```

```{r 10_plot_flipper_bill-code-check}
grade_code()
```
...
```{r 12_create_years_factor, exercise=TRUE, exercise.setup = "pre_penguin"}
```

```{r 12_create_years_factor-solution}
data("penguins")
penguins$year <- factor(penguins$year)
levels(penguins$year)
```

```{r 12_create_years_factor-code-check}
grade_code()
```
...
```{r 14_plot_flipper_bill_by_year, exercise=TRUE, exercise.setup = "pre_year_colors"}
# leave the next line first; add code below the next line
year_colors <- c("red", "gold", "blue")

```

```{r 14_plot_flipper_bill_by_year-solution}
year_colors <- c("red", "gold", "blue")
data("penguins")
penguins$plot_color <- year_colors[penguins$year]
plot(
...
```{r 14_plot_flipper_bill_by_year-code-check}
grade_code()
```
...
```{r 15_colors_correct, exercise=TRUE, exercise.setup = "pre_year_colors"}

```

```{r 15_colors_correct-solution, exercise.reveal_solution=FALSE}
dplyr::distinct(penguins[, c("year", "plot_color")])
```

```{r 15_colors_correct-check}
grade_this( {
  pass_if(
    TRUE,
...
```{r 16_plot_flipper_bill_by_island, exercise=TRUE, exercise.setup = "pre_year_colors"}

```

```{r 16_plot_flipper_bill_by_island-solution, exercise.reveal_solution=FALSE}
...
```{r 16_plot_flipper_bill_by_island-check}
grade_this( {
  pass_if(
    TRUE,
    message = "Your solution has been submitted for grading.") })
...
- Then plot bill length vs. flipper length again, but this time with the different colors representing different species.



```{r 17_plot_flipper_bill_by_species, exercise=TRUE, exercise.setup = "pre_year_colors"}

```

```{r 17_plot_flipper_bill_by_species-solution, exercise.reveal_solution=FALSE}
...
```{r 17_plot_flipper_bill_by_species-check}
grade_this( {
  pass_if(
    TRUE,
    message = "Your solution has been submitted for grading.") })
...
```{r 18_interpretation_quiz}
quiz(
  question_text(
    "What do you conclude about the relationship between flipper length and bill length *within* each species?",
    answer_fn(tutorialize::submitToTA),
    rows = 2,
    correct = "Thanks. The TA will evaluate your answer.  One possible correct answer would be something like 'Within each species, the bill length is positively correlated with flipper length and the slope of the relationship is similar regardless of species.'"),
  question_text(
    ...
    answer_fn(tutorialize::submitToTA),
    rows = 4,
    correct = "Thanks. The TA will evaluate your answer.  A correct answer might include observations such as 'Adelie and Chinstrap penguins have roughly the same range of flipper length, but Chinstraps have overall, longer bills that Adelies.  Gentoos have longer flippers than the other two species, but the range in bill length is similar to Chinstraps.  The ratio of flipper length to bill length is similar for Adelie and Gentoo penguins'"),
  question_text(
